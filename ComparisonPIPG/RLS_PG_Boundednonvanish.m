...
Ahistory=zeros(nx*nx,episode);
Bhistory=zeros(nx*nu,episode);
PK=10*eye(nx+nu);  % initial matrix PK=H_t^{-1}
%% iterations of PE and PI(basic parameters)
xhistory=zeros(nx,episode);
uhistory=zeros(nu,episode);
...
for j=1:episode
    ...
    %% RLS
        ...
        % u=uforward+K_current*xhistory(:,j); 
        % uhistory(:,j)=u;
        % xhistory(:,j+1)=realsystem(A,B,xhistory(:,j),u,noise(1,j)); % Collect the data d_t
        % [A_est,B_est,PK]=RLS(A_est,B_est,PK,xhistory(:,j),xhistory(:,j+1),u); % RLS
        % Ahistory(:,j)=A_est(:);
        % Bhistory(:,j)=B_est(:);
    %% Policy Gradient
