...
Ahistory5=zeros(nx*nx,episode);
Bhistory5=zeros(nx*nu,episode);
PK5=10*eye(nx+nu);  % initial matrix PK=H_t^{-1}
%% iterations of PE and PI(basic parameters)
xhistory5=zeros(nx,episode);
uhistory5=zeros(nu,episode);
khistory5=zeros(nx*nu,episode);
xhistory5(:,1)=xstart;
K_initial=-K_initial;
khistory5(:,1)=K_initial(:);
K_current=K_initial;
phistory5=zeros(nx*nx,episode);
...
for j=1:episode
    ...
    %% RLS
        ...
        % u=uforward+K_current*xhistory5(:,j); 
        % uhistory5(:,j)=u;
        % xhistory5(:,j+1)=realsystem(A,B,xhistory5(:,j),u,noise(1,j)); % Collect the data d_t
        % [A_est,B_est,PK5]=RLS(A_est,B_est,PK5,xhistory5(:,j),xhistory5(:,j+1),u); % RLS
        % Ahistory5(:,j)=A_est(:);
        % Bhistory5(:,j)=B_est(:);
    %% Policy Gradient
    khistory5(:,j)=vec(K_current);   
...
save("RLS_PG_Boundedto0")
subplot(2,1,1);
plot(Normalized_data(phistory5,Pstar),'linewidth',1,'color','green');
...
subplot(2,1,2)
plot(Normalized_data(khistory5,-Kstar),'linewidth',1,'color','green');
xlabel('$\mathrm{iteration~index}$','interpreter','latex','FontSize',12);
