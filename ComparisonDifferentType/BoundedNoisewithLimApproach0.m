...
Ahistory1=zeros(nx*nx,episode);
Bhistory1=zeros(nx*nu,episode);
PK1=10*eye(6);  % initial matrix PK=H_t^{-1}
%% iterations of PE and PI(basic parameters)
% Bounded noise with lim approaching 0
xhistory1=zeros(nx,episode);
uhistory1=zeros(nu,episode);
khistory1=zeros(nx*nu,episode);
xhistory1(:,1)=xstart;
khistory1(:,1)=K_initial(:);
K_current1=K_initial;
phistory1=zeros(nx*nx,episode);
factor=10; % additional excitation
%% Start Algorithm
for j=1:episode
    %% Bounded noise with lim approaching 0
    % Policy Evaluation
    Pmb=dlyap((A_est+B_est*K_current1)',Q+K_current1'*R*K_current1); % policy evalutaion based on estiamtes (\hat{A}_{i-1},\hat{B}_{i-1})
    phistory1(:,j)=Pmb(:);
    % RLS
    ...
    u=uforward+K_current1*xhistory1(:,j); 
    uhistory1(:,j)=u;
    xhistory1(:,j+1)=realsystem(A,B,xhistory1(:,j),u,noise(1,j)); % Collect the data d_t
    [A_est,B_est,PK1]=RLS(A_est,B_est,PK1,xhistory1(:,j),xhistory1(:,j+1),u); % RLS
    Ahistory1(:,j)=A_est(:);
    Bhistory1(:,j)=B_est(:);
    % Policy improvement
    khistory1(:,j)=vec(K_current1);    
    K_current1=-(R+B_est'*Pmb*B_est)\B_est'*Pmb*A_est;  % policy improvement based on estiamtes (\hat{A}_{i},\hat{B}_{i})

    %%
end
save("BoundedNoiseApproach0");
subplot(2,1,1);
plot(Normalized_data(phistory1,Pstar),'linewidth',1,'color','green');
...
subplot(2,1,2)
plot(Normalized_data(khistory1,-Kstar),'linewidth',1,'color','green');
xlabel('$\mathrm{iteration~index}$','interpreter','latex','FontSize',12);
